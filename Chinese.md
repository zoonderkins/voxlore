...
- **聽懂意圖，不只逐字轉錄。**
- **科技、法律、日常語境都能穩定整理成可用文字。**
- **支援繁中混英與台灣常見外來語發音（如 `baku` → 後退 / back）。**

### 長版

- **懂意圖，不只聽字面**：口語講法會自動整理成可用句子，不只是逐字抄錄。
- **科技語言友善**：技術詞、英文術語、縮寫、API 名稱盡量保留原貌，不亂翻。
- **法律語氣穩定**：偏正式與精確措辭可維持語意一致，降低改寫失真。
- **常識修正**：自動補標點、去贅詞、修正常見口誤，讓輸出更接近可直接貼上的文本。
- **台灣混語適配**：對繁中混英與台灣常見外來語發音更友善，例如 `baku` 可依語境理解為「後退 / back」。
...
- 自動回焦目標 App 並貼上
- 文字增強（標點、格式、語句整理）
- 文字增強強制採用 speech-to-text 情境，避免回覆「請提供影片連結/檔案」
- 浮動小視窗顯示錄音與處理狀態
